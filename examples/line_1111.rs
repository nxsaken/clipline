    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        for (p, q) in [(p1, p2), (p2, p3), (p3, p4), (p4, p1)] {
            ...
            line.for_each(|(x, y)| {
                ...
                // Calculate the index in the flat vector
                let index = y as usize * WIDTH + x as usize;
                buffer[index] = 0x00FF00; // Set the color to white
            });
        }
...
fn draw_rectangle(buffer: &mut Vec<u32>) {
    let start_x = (-32 + 64) as usize; // Convert to positive index
    let start_y = (-32 + 64) as usize; // Convert to positive index
    ...
    for y in start_y..=end_y {
        for x in start_x..=end_x {
            let index = y * WIDTH + x;
            if index < buffer.len() {
                buffer[index] = 0x0000FF; // Set the color to white
            }
        }
    }
}
